...
class PPO():
    ...
    def optimize(self):
        ...
        old_action_log_prob = tf.constant(
            [t.a_log_prob for t in self.buffer], dtype=tf.float32)
        ...
        for _ in range(round(10*len(self.buffer)/batch_size)):
            ...
            index = np.random.choice(
                np.arange(len(self.buffer)), batch_size, replace=False)
            # 构建梯度跟踪环境
            with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
                ...
                surr2 = tf.clip_by_value(
                    ratio, 1 - epsilon, 1 + epsilon) * advantage
                # PPO误差函数
