...
int main()
{
    ...
    network.nn << tiny_dnn::fully_connected_layer(1, 10)
               << tiny_dnn::tanh_layer()
               << tiny_dnn::fully_connected_layer(10, 10)
               << tiny_dnn::tanh_layer()
               << tiny_dnn::fully_connected_layer(10, 1);

    rlcpp::TinyDNN_Network<tiny_dnn::mse> other;
    other.nn << tiny_dnn::fully_connected_layer(1, 10)
             << tiny_dnn::tanh_layer()
             << tiny_dnn::fully_connected_layer(10, 10)
             << tiny_dnn::tanh_layer()
             << tiny_dnn::fully_connected_layer(10, 1);
