...
std::tuple<double, Int> run_episode(Env& env, DQN_agent& agent, State& obs, State& next_obs, Action &action, Action& next_action, double &reward, bool &done, bool bRender = false)
{
    Int total_steps = 0;
...
void test_episode(Env& env, DQN_agent& agent, State& obs, State& next_obs, Action &action, double &reward, bool &done) 
{
    double total_reward = 0.0;
    env.reset(&obs);
    while (true)
