...
void train_pipeline_progressive(Env &env, DQN_TinyDNN_agent &agent, Float score_threshold, Int n_episode, Int learn_start = 100, Int print_every = 10)
{
...
void train_pipeline_conservative(Env &env, DQN_TinyDNN_agent &agent, Float score_threshold, Int n_epoch = 500, Int n_rollout = 100, Int n_train = 1000, Int learn_start = 0, bool early_stop = true)
{
    auto obs = env.obs_space().getEmptyObs();
    auto next_obs = env.obs_space().getEmptyObs();
    auto action = env.action_space().getEmptyAction();
...
void test(Env &env, DQN_TinyDNN_agent &agent, Int n_turns, bool render = false)
{
