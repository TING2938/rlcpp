...
public:
    ...
    void getArray(std::vector<T>& x, std::string str, int length, bool required, std::string msg)
    {
        ...
        ss << "]";
        if (addHelpInfo(required, str + (length ? "(len:" + std::to_string(length) + ")" : "(len:auto)"), ss.str(),
                        msg))
            return;
        ...
        if (b != argv.end()) {
            x.clear();
            auto beg = std::find_if(b + 1, argv.end(), [](const std::string& chr) { return chr.front() == '['; });
            if (beg == argv.end()) {
                std::fprintf(stderr, "Error of command %s!\n", str.c_str());
                std::exit(-1);
            }
            auto end = std::find_if(b + 1, argv.end(), [](const std::string& chr) { return chr.back() == ']'; });
...
private:
    ...
    bool addHelpInfo(bool required, std::string str, const T& x, std::string msg)
    {
        if (_printHelpMain || _printHelpSub) {
            ...
            ss << std::setiosflags(std::ios::left) << std::string(5, ' ') << std::setw(15) << str << std::setw(30) << x
               << msg << "\n";
            if (required) {
