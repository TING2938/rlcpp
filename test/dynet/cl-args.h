...
static void usage(std::ostream& os, const char* program, Task task)
{
    if (task == TRAIN)
        ...
        os << "usage: " << program << " -t train_file -d dev_file -tl train_labels_file -dl dev-labels_file [options]"
           << std::endl;
    else if (task == TEST)
 ...
 * \param task Task
 */
void get_args(int argc, char** argv, Params& params, Task task)
{
    int i = 1;
    while (i < argc) {
        string arg = argv[i];
    ...
    if (task == TRAIN) {
        ...
        if (params.train_file == "" || params.dev_file == "" || params.train_labels_file == "" ||
            params.dev_labels_file == "") {
            stringstream ss;
            usage(ss, argv[0], task);
            throw invalid_argument(ss.str());
        }
